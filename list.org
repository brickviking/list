...
** Watcom response file
   ...
   :END:
I've found out that DOS has a 128-char limit, so the Watcom tools use response files to pass more information than the paltry amount of 128 bytes around. Hence, here's a list.lnk file that Watcom's wmake program can use. 
...
Putting back the debug stuff into Watcom's response file requires a separate response file for the debug objects. Additionally, the filenames all need to fit within 8.3, or Watcom's tools simply can't find them, at least under FreeDOS even with long filename support turned on—which it is by default anyhow. They can't even output anything that's not 8.3, which somewhat surprises me.
...
This maybe needs to be turned into MSDOS-guacamole, i.e. with ^M at the ends of every line, although Watcom's wmake doesn't seem to complain about it.
