...
Later, I gained some experience with programming for cross-platform programs. I learned how to make
list work under most modern BSDs, Solaris (probably), Minix, and Linux from 2.0.x onwards, as well
as DJGPP. All you'll need will be some version of curses or ncurses, and the standard libraries.
...
(1) Well, large-scale for me, anyhow. I tend to like single-file projects. This started off as a
single-file project, but then it was deliberately split across multiple files as a bit of an
exercise.
...
emacs to write this up, and you can extract all the source files for the project from just this one
file.
...
manpages. I'm also including the README.md file whole, as well as the Changelog in the src/
directory.

I think I've got 100% of the source files in this org document that you will need to re-create the
project as it stands, assuming that you grabbed this from the github repository. I probably won't
include this file itself in the tarballs, as that would be completely redundant. After all, who
needs /two/ copies of the same thing?

The only thing you won't get from this single org file are the original SVG images for the badges,
which were added to show compile status for the project on my own machine, and were initially
supplied by [Shields.io](https://shields.io). I don't have NetBSD or Solaris installed at the
moment, and I don't see me getting to them in the near future. However, I have compiled them on the
rest of the operating systems I state. Your mileage might vary for cygwin or WSL, but it's worth a
try.
...
** AMA files for AMBHELP (FreeDOS help system)
And to go with the man page, here's a couple of files that describe how list works, at least under
a FreeDOS setting. There doesn't seem to be an equivalent for MS-DOS, at least not one that doesn't
use the QBASIC or QuickBasic environment.

#+BEGIN_SRC text :tangle src/ambhelp/INDEX.AMA -i
%hLIST for MSDOS/FreeDOS

list is a curses-based interactive file listing program, which displays
text on the screen in one of two different formats, either in
hexadecimal dump format (a la MS-DOS debug.com format), or in text
format, similar to what the programs
%lless %tand
%lmore %tdo. More or less.
...
list understands the following command line options.  Only the short form
options are available in this version. It remains for me to write the
code to understand the long format switches.

    "-f filename"
specifies what file to display.  The "-f" switch is not necessary, however,
the filename %!is%t mandatory, else list will just quit back to an empty
screen, and complain that it can't open an empty file.  I have not (yet)
sorted out input from a pipe, so input from stdin is out.

    "-d"
Specifies the debug switch.  This is not very useful unless you want to press
a key for each iteration through a line.  Use as you want.

    "-x"
Display the file in hexdump format (as in MS-DOS DEBUG.COM format) except the
display will take up the width of the screen, instead of being limited to 80
chars.  This looks cool on a 160x64 screen.

    "-t"
Display the file in text format, just like other pagers (less and more).

    "-q"
Quickmode - only displays in hexmode, and skips the scanning for line ends.
This dramatically speeds up file startup for those of us who don't need a text
dump - for example, in large 125 meg files.  I have already used this program
on such a file, and was quite happy with the result.  However, if you really
want the whole darn thing displayed as text, then it will take a long time.
This is something I have to learn too.

    "-7"
Display the file using 7 bit ASCII.  All characters above 0x127 will be
displayed as ".", as well as any chars less than 0x20 (space).  This is the
default dump mode for the program. This is also the common ground for most
Unices and DOS.  Windows, I don't really know about, but suspect it's the
same.

    "-8"
Display the file using the extended ASCII 8-bit table.  This will display all
characters except ones with a value less than 0x20 (space).  These latter are
displayed as "." - however, this causes the screen to behave very strangely
with certain files, usually binary files. Because of the different way that
systems choose to show these values, you could get differing results between
machines or even codepages.

    "-e"
Show line endings in text mode. This specifically shows the %!"CR"%t as a
period. If the file originated from a Windows, Mac or DOS system, it will
already be showing the linefeed character as a period.
...
future to reflect real lines, such as less presents to the user.
...
.I "TERMINFO"
This may need to be set if the path to the fIterminfofP database is
different between the system the binary was compiled on and the one it
is being run on.  For example, in older Slackware systems, the
fIterminfofP database resides in fI/usr/lib/terminfofP.  On RedHat
Linux, it is in fI/usr/share/terminfofP.  If the program complains
about the terminal type, and the value of fITERMfP is correct, check
into this.  Also check if the output looks really weird - for example
if you get funny looking 8 bit output on a system that you know can
display 8 bit characters properly.
...
The original fBlistfP program that I saw started out as a program
under MS-DOS, created by Vernon Buerg, to whom I acknowledge the
initial idea.  However, there was not an equivalent program under
Linux, though people have tried.  There is a hexdump program, which
provides something like what I wanted, but I also wanted to have a
...
%hBUGS, ERRORS AND OMISSIONS

list is most definitely still beta software. it is also only designed to
reliably handle 7-bit printable ASCII characters, at least under UNIX
(this includes Linux).  8-bit and non-printing ASCII characters in
files may not be correctly handled, though this will depend on what or
where you compile and I have tried to sort out the worst offenders
...
Technically I don't need this, as it's already in the system.

#+BEGIN_SRC text :tangle src/ambhelp/LESS.AMA
%hless, a pager for MS-DOS

I'm sure I don't need to point you to an actual example, but if you're totally
unaware of less, then it's the UNIX/Linux/BSD equivalent of more, but with way
more features than more has ever had. The tradeoff for this is that it's way
larger. That's not usually a problem for modern machines, but more will work
on memory-constrained machines that have 1MB of memory on board. In comparison
less might need a bit more memory available to it, even on DOS.

%lMORE.AMA:Description for MORE
%lLIST.AMA:Back to previous document
#+END_SRC

And here's a page for MORE, which is also in the system.

#+BEGIN_SRC text :tangle src/ambhelp/MORE.AMA
%hMore - a pager for MS-DOS

More is a pager, which will display a text file page-by-page on your screen,
much like pages on a printer.
%lLIST.AMA:Back to previous document
#+END_SRC
...
under Minix. How does that differ from BSDMake or GNU Make? Not much, apparently. I can now feed
the BSDMakefile into what passes for make on Minix now, and it all just works.
...
So far, these both need pdcurses. There's other work to do to get it to work under Watcom.

** Watcom response file
   :PROPERTIES:
   :header-args: :tangle src/list.lnk
   :END:
I've found out that DOS has a 128-char limit, so the Watcom tools use response files to pass more information than the paltry amount of 128 bytes around. Hence, here's a list.lnk file that Watcom's wmake program can use. There's no debugging symbols included, that's left to the listd.exe object.
...
#+END_SRC
** Watcom debug response file
   :PROPERTIES:
   :header-args: :tangle src/list_d.lnk
   :END:

Putting back the debug stuff into Watcom's response file requires a separate response file for the
debug objects. Additionally, the filenames all need to fit within 8.3, or Watcom's tools simply
can't find them, at least under FreeDOS even with long filename support turned on—which it is by
default anyhow. They can't even output anything that's not 8.3, which somewhat surprises me. This
time I've turned on debug symbols.
...
option map
debug Watcom All
file { listd.obj dequeued.obj bgwrited.obj filed.obj inputd.obj shwhelpd.obj screend.obj }
library { c:\pdcurses\dos\pdcurses.lib }
#+END_SRC
...
This maybe needs to be turned into MSDOS-guacamole, i.e. with ^M at the ends of every line, although
Watcom's wmake doesn't seem to complain about it. We don't want the debug executable made by default, the user should know to create this, and if they don't deliberately create it, it shouldn't be made.
...
+ What happened to the colours? That seems to have been fixed now, I think.
