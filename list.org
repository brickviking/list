...
** Changelog
   :PROPERTIES:
   :header-args: :tangle ChangeLog
   :END:


Okay, okay. I can see the sense of including this now, but all I really need to do is pull it in via
a source block. There's just one small problem - it's in C comment at the moment, but it really
doesn't need to be in that. I can really just include it as straight text.
...
#+BEGIN_SRC text
  ...
  0.7.34  Many formatting changes, centred around spacing and consistency.
         ...
         Haven't bumped version yet, will do that when everything is done.
